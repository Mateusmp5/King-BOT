    ...
    try {
        ...
        case 'stiker':
            if (isMedia && isImage) {
				...
				await kill.sendImageAsSticker(from, `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`, { author: config.author, pack: config.pack, keepScale: true })
            } else if (args.length == 1) {
                if (isUrl(url)) {
                    await kill.sendStickerfromUrl(from, url, { method: 'get' }, { author: config.author, pack: config.pack, keepScale: true })
        ...
        case 'nobg':
			if (isMedia && type === 'image' || isQuotedImage) {
				...
				await kill.sendImageAsSticker(from, `data:${nobgmd.mimetype};base64,${result.base64img}`, { author: '🎁 https://wa.me/5516993761686 ☆', pack: 'Asta BOT ☆', keepScale: true })
        ...
        case 'gif':
			if (isMedia && isVideo || isGif || isQuotedVideo || isQuotedGif) {
				...
				await kill.sendMp4AsSticker(from, mediaData, null, { stickerMetadata: true, pack: config.pack, author: config.author, fps: 10, crop: true, loop: 0 })
       ...
       case 'clima':
		...
		case 'stickers':
			await kill.sendText(from, mess.stickers(pushname, time))
			break
			
			
		case 'otaku':
			await kill.sendText(from, mess.anime(pushname, time))
			break
			
			
		case 'games':
			await kill.sendText(from, mess.games(pushname, time))
			break
        ...
        case 'adult':
            if (isGroupMsg && !isNsfw) return kill.reply(from, mess.gpadulto(), id)
            await kill.sendText(from, mess.adult(pushname, time))
            break
        ...
        case 'down':
            await kill.sendText(from, mess.down(pushname, time))
            break
			
			
        case 'dados':
            await kill.sendText(from, mess.dados(pushname, time))
            break
			
			
        case 'midia':
            await kill.sendText(from, mess.midia(pushname, time))
            break
